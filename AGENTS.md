...
| DESIGN.md | Architecture decisions D1–D23 — read before changing structure |
| docs/status.md | Where the build actually is |

## Rules

- `make help` lists everything. `make install && make test` from a fresh clone.
- **DESIGN.md is frozen.** Implement against D1–D23. If implementation exposes a
  genuinely missing decision, record it there as a new decision — never decide
  silently in code. D20–D22 were added that way after Step 0, D23 after Step 1.
